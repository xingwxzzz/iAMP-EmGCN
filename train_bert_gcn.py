...
parser.add_argument('-m', '--m', type=float, default=0.2, help='the factor balancing BERT and GCN prediction')
parser.add_argument('--nb_epochs', type=int, default=30)
parser.add_argument('--bert_init', type=str, default='roberta-base',
                    choices=['roberta-base', 'roberta-large', 'bert-base-uncased', 'bert-large-uncased'])
parser.add_argument('--pretrained_bert_ckpt', default=None, choices=['./checkpoint/roberta-base_AMPs/checkpoint.pth'])
parser.add_argument('--dataset', default='AMPs', choices=['20ng', 'R8', 'R52', 'ohsumed', 'mr', 'AMPs'])
...
# Data Preprocess
adj, features, y_train, y_test, train_mask, test_mask, train_size, test_size = load_corpus(dataset)
'''
...
nb_train, nb_test = train_mask.sum(), test_mask.sum()
...
corpse_file = './cross_validation_data/fold_9/data/corpus/' + dataset +'_shuffle.txt'
...
g.ndata['label'], g.ndata['train'], g.ndata['test'] = \
    th.LongTensor(y), th.FloatTensor(train_mask), th.FloatTensor(test_mask)
...
doc_idx = Data.ConcatDataset([train_idx,test_idx])
...
def log_training_results(trainer):
    ...
    train_confusion = metrics["confusion"]

    evaluator.run(idx_loader_test)
    metrics = evaluator.state.metrics
    test_acc, test_nll = metrics["acc"], metrics["nll"]
    test_confusion = metrics["confusion"]

    logger.info(
        "Epoch: {}  Train acc: {:.4f} loss: {:.4f} Test acc: {:.4f} loss: {:.4f}"
        .format(trainer.state.epoch, train_acc, train_nll, test_acc, test_nll)
    )
    ...
    if test_acc > log_training_results.best_test_acc:
        ...
        TP = test_confusion[1][1]  # 真正例
        FN = test_confusion[1][0]  # 假负例
        TN = test_confusion[0][0]  # 真负例
        FP = test_confusion[0][1]  # 假正例
        # 计算特定性
        sp = TN / (TN + FP)
        ...
        logger.info("Test SP...")
        logger.info(sp)
        logger.info("Test MCC...")
        mcc = (TP * TN - FP * FN) / math.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN))
        logger.info(mcc)
        logger.info("Test Precision, Recall and F1-Score...")
        logger.info(test_precision)
        logger.info(test_recall)
        f_score = (test_precision * test_recall * 2 / (test_precision + test_recall))
        logger.info(f_score)
